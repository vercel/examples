...
export const WelcomeView = ({
  ...
  const handleStartCall = async () => {
    if (useVercelSandbox) {
      // Create sandbox (will be no-op if already exists due to guard in createSandbox)
      await createSandbox();
      // onSandboxReady will be called by the useEffect when status becomes 'ready'
    } else {
      // Standard flow without sandbox
      onStartCall();
    }
  };

  // Auto-connect when sandbox is ready (for both new and resumed sandboxes)
  ...
  useEffect(() => {
    if (
      ...
      lastConnectedSandboxRef.current = sandbox.id;
      // Notify parent about the sandbox (works for both new and resumed)
      onSandboxReady(sandbox.id, sandbox.roomName || '');
    }

    // Reset the flag when status changes from ready (e.g., when user disconnects)
    if (status === 'idle') {
  ...
  // Show loading state during sandbox creation or when checking for existing sandbox
