...
async function getFlags() {
  ...
  const factory = SplitFactory({
    core: {
      authorizationKey: process.env.SPLIT_SDK_API_KEY!,
    },
  })

  const client = factory.client()
  await new Promise((res) => client.on(client.Event.SDK_READY, res))

  const key = 'user_123abc'

  const flags = {
    docs: overrides?.docs ?? client.getTreatment(key, 'docs'),
    learn: overrides?.learn ?? client.getTreatment(key, 'learn'),
