  ...
  return (
    <Page>
      <Head>
        ...
        <meta name="description" content="How to adapt data fetching to avoid moving from static pre-rendering" />
        <link rel="icon" href="/favicon.ico" />
      </Head>

      <section className="flex flex-col gap-6">
        <Text variant="h1">Combining data fetching strategies</Text>
        <Text>Next.js has two forms of pre-rendering: Static Generation and Server-side Rendering. The difference is in when it generates the HTML for a page.</Text>
        <List>
      ...
      <section className="flex flex-col gap-3">
        ...
        <Text>We have an e-commerce product page that is statically generated, but we need to disable the "Add to cart" button if we have no stock. We can combine our data fetching strategies to get the best of both worlds. We will still generate our product page statically but we will fetch our stock periodically to update our UI.</Text>
        <Image src={board} />
        <Text>This page is statically generated so you can see the example below. We are using <Link href="https://swr.vercel.app">SWR</Link> to fetch the stock status every 5 seconds and disable the "Add to cart" button if we have no stock. The stock is a random number so you can see the button getting disabled and enabled several times.</Text>
      </section>
