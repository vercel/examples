    ...
    const handleMint = async () => {
        try {
            ...
            if (!Moralis.Plugins?.rarible)
                throw new Error('Please install Rarible Plugin to your Moralis Server: https://moralis.io/plugins/rarible-nft-tools/');
            ...
            setTimeout(() => {
                router.push(`https://rinkeby.rarible.com/token/${data.result.tokenAddress}:${data.result.tokenId}`);
            }, 1000);
        } catch (e) {
            console.error(e);
        } finally {
        ...
        <div className="inline-block align-bottom text-left overflow-hidden  transform transition-all sm:my-8 sm:align-middle  ">
            ...
            {state === MintState.ConfirmMint && (
                <>
                    ...
                    <Text className="mt-6">
                        Your image will be minted as an ERC721 Token. It can happen that images stored on IPFS as it is a distributed file
                        hosting system that can fail. This is still the prefered method of choice to host in the NFT community as it is
                        decentralized. <span className="underline italic">This process might take up to 1 minute to complete</span>
                    </Text>
                    ...
                    <section className="flex justify-center mt-6">
                        <Button size="lg" variant="black" onClick={handleMint} disabled={!account || !asset || isLoading}>
