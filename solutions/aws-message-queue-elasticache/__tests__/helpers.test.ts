...
import { validateContactForm, fieldsToRecord } from '../app/api/messages/helpers'
...
describe('validateContactForm', () => {
  ...
  it('rejects missing email', () => {
    ...
    expect(result).toEqual({ valid: false, error: 'Email is required and must be a string' })
  })

  it('rejects missing message', () => {
    const result = validateContactForm({ name: 'A', email: 'a@b.com' })
    expect(result).toEqual({ valid: false, error: 'Message is required and must be a string' })
  })

  it('rejects non-string name', () => {
    const result = validateContactForm({ name: 123, email: 'a@b.com', message: 'hi' })
  ...
  it('rejects name over 200 chars', () => {
    ...
    expect(result).toEqual({ valid: false, error: 'Name must be 200 characters or fewer' })
  })
  ...
  it('rejects email over 320 chars', () => {
    ...
    expect(result).toEqual({ valid: false, error: 'Email must be 320 characters or fewer' })
  })
  ...
  it('rejects message over 10000 chars', () => {
    ...
    expect(result).toEqual({ valid: false, error: 'Message must be 10000 characters or fewer' })
  })
