...
One of the popular use cases for MF we hear often is runtime injection of code, done with the intention of updating the JS assets of a site without rebuilding the site, to improve development speed (DX) and ship faster, however, before diving into that route we highly recommend you to consider the following downsides of having independent build systems and merging them together:

- There shouldn't be a way to ship new JS to your site without first testing the site as a whole, which requires building the site, deploying it and going through checks (CI/CD), or otherwise you'll risk breaking the site with every live change and it can get exponentially worse as teams grow. We recommend you always build your whole app and run your entire CI/CD pipeline to keep your teams in [the pit of success](https://blog.codinghorror.com/falling-into-the-pit-of-success/)
...
- Upgrading versions can prove to be a painful task especially on large teams where you'll need to wait for multiple approval of changes in apps outside your scope, defeating the initial purpose of improving DX.
  - Teams might start to diverge and ship their own libraries in their preferred versions, this can lead to UX and DX issues as build size issues that are hard and slow to solve start to come up
- MF often involves merging builds (JS scripts) in the browser, this is bad for performance because it means users need to wait for the browser to execute JS before interacting with the page, however it makes sense, because if you wanted to do something similar for pregenerated content (SSG, SSR) you would also need to do live changes to lambda functions, the CDN, Proxy, and likely more, increasing the overall complexity
- [Multi Zones](#multi-zones) has an important constraint that stops if from falling easily in the issues mentioned above: The app has total control over the paths it's in charge of, so it's only when you navigate to paths owned by another app that you can notice a slowdown

Module Federation can be an improvement in apps that are built differently and want to merge together, but if your teams all build on the same stack, we recommend taking a look at the approaches above first.
